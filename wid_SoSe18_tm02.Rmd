...
author: "Alexander Kojen, "  # Hier Die Nammen aller Gruppenmitglieder eintragen!
output: pdf_document
---
...
length(tables)
noReviews <- dbGetQuery(mydb, 'SELECT Count(score) FROM reviews')
print(noReviews)
...
noBeatlesReviews <- dbGetQuery(mydb, 'SELECT Count(score) FROM reviews WHERE artist = "the beatles"')
print(noBeatlesReviews)
no50centReviews <- dbGetQuery(mydb, 'SELECT Count(score) FROM reviews WHERE artist = "50 cent"')
...
avgBeatlesReviews <- dbGetQuery(mydb, 'SELECT avg(score) FROM reviews WHERE artist = "the beatles" or artist = "50 cent"')
...
list20ReviewArtists <- dbGetQuery(mydb, 'SELECT DISTINCT(rev.artist) FROM reviews rev WHERE (SELECT COUNT(score) FROM reviews WHERE artist = rev.artist) >= 20')
...
```{r}
##Antwort:
#Wenn man sich die Top 3 der häufigsten Mengen anschaut, dann erkennt man z.B. dass Oasis Alben am häufigsten dem Genre Rock angehören und dahingehend ein Zusammenhang besteht (Oasis -> Genre Rock). Dagegen erkennt man im zweiten Datensatz, dass die mit 8/10 und besser bewerteten Alben zumeist nicht mit dem Titel "best new music" (bnm) versehen wurden und dass die Review von sog. "nontributors" verfasst wurden. Auch dahingehend besteht ein Zusammenhang (Nicht bnm -> Nicht contributor). Dementsprechnd ergibt sich durch Interpretationsspielraum in den gefundenen Mengen.
