...
train.mylda <- function(data, target) {
  ...
  for (i in 1: length(classes)) {
    ...
    n <- nrow(cleanClassSpecificData) ###Anzahl der p-dimensionalen Beobachtungen. Muss kein Vektor sein, oder? 
    meanVec <- apply(cleanClassSpecificData, 2, mean) #Parameter 2: spaltenweise
    meanList <- c(meanList, meanVec)
  
    for (j in 1:n){ #Pro Beoabchtung wird summiert nach Formel (13), s. 143
  ...
  n <- nrow(cleanData) ###Hier müsste man aus meiner Sicht wieder die Gesamtanzahl der Beobachtungen nehmen
  
  meanMat <- matrix(meanList, ncol=p)
  meanFrame <- data.frame(meanMat, classes)
  colnames(meanFrame) <- colnames(data)
  
  firstPartCov <- (1/ (n-length(classes)))
  covMat <- firstPartCov*matSum
  
  return( list("cov"=covMat, "means"=meanFrame, "apriori"=1 / length(classes)) ) ###Apriori-Wahrscheinlichkeit einer der Klassen zugeordnet zu sein.
}
...
predict.mylda <- function(model, newdata) {
  ...
  for (i in 1:nrow(cleanNewdata)){ #Für jede Test-Beobachtung
    predResult[i,1] <- toString(newdata[i, 5]) #(Wahre Klasse wird schon im Voraus eingetragen) - To do: non-numeric Index dynamisch bestimmen
    maxH <- -Inf
    for (j in 1:nrow(model$means)){ #Und jede Klasse soll Formel (6), s. 130 berechnet werden
      ...
      first <- t(invCov %*% classSpecificMeanVector) %*% as.numeric(cleanNewdata[j,])
      second <- 0.5*classSpecificMeanVector %*% invCov %*% classSpecificMeanVector
      third <- log( model$apriori , base = exp(1))
      ges <- first-second+third
      sum(ges)
      if (sum(ges) >= maxH) {
        predResult[i,2] <- toString(model$means[j,5]) #(Vorhergesate Klasse wird ergänzt) 
        maxH <- sum(ges)
      }
    }
  }
...
eval
paste(c("Mean misclassification error (Fehlerklassifikationsrate): ", round(eval[1,2]*100,digits= 2), "%"), collapse = "")
...
param_results <- as.data.frame(rda_param_eval(gamma, lambda))
param_results <- as.matrix(param_results[order(-param_results$Fehlklassifikationsrate),])
print(param_results)
```
